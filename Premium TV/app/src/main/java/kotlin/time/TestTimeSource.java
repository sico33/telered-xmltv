...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1741plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1653toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != Long.MAX_VALUE) {
            ...
            double delta = Duration.m1650toDoubleimpl(duration, getUnit());
    ...
    private final void m1740overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + ((Object) Duration.m1656toStringimpl(duration)) + '.');
    }
}
