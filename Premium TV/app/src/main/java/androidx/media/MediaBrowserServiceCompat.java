...
public abstract class MediaBrowserServiceCompat extends Service {
    ...
    class MediaBrowserServiceImplApi21 implements MediaBrowserServiceImpl {
        ...
        void setSessionTokenOnHandler(MediaSessionCompat.Token token) {
            if (!this.mRootExtrasList.isEmpty()) {
                ...
                if (extraBinder != null) {
                    for (Bundle rootExtras : this.mRootExtrasList) {
        ...
        public BrowserRoot onGetRoot(String clientPackageName, int clientUid, Bundle rootHints) {
            ...
            if (rootHints != null && rootHints.getInt(MediaBrowserProtocol.EXTRA_CLIENT_VERSION, 0) != 0) {
                ...
                int clientPid2 = rootHints.getInt(MediaBrowserProtocol.EXTRA_CALLING_PID, -1);
                rootHints.remove(MediaBrowserProtocol.EXTRA_CALLING_PID);
                clientPid = clientPid2;
            } else {
                clientPid = -1;
            }
            ConnectionRecord connection = MediaBrowserServiceCompat.this.new ConnectionRecord(clientPackageName, clientPid, clientUid, rootHints, null);
    ...
    private static final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (this.mService != null) {
                this.mService.handleMessageInternal(msg);
            } else {
        ...
        public void registerCallbacks(final ServiceCallbacks callbacks, final String pkg, final int pid, final int uid, final Bundle rootHints) {
            MediaBrowserServiceCompat.this.mHandler.postOrRun(new Runnable() { // from class: androidx.media.MediaBrowserServiceCompat.ServiceBinderImpl.6
                ...
                public void run() {
                    ...
                    IBinder b = callbacks.asBinder();
                    MediaBrowserServiceCompat.this.mConnections.remove(b);
                    ConnectionRecord connection2 = null;
                    ...
                    while (iter.hasNext()) {
                        ...
                        if (pendingConnection.uid == uid) {
                            if (TextUtils.isEmpty(pkg) || pid <= 0) {
                                connection2 = MediaBrowserServiceCompat.this.new ConnectionRecord(pendingConnection.pkg, pendingConnection.pid, pendingConnection.uid, rootHints, callbacks);
                            }
                            iter.remove();
                            break;
                        }
                    ...
                    if (connection2 != null) {
                        ...
                        connection = MediaBrowserServiceCompat.this.new ConnectionRecord(pkg, pid, uid, rootHints, callbacks);
                    }
                    MediaBrowserServiceCompat.this.mConnections.put(b, connection);
                    try {
                        b.linkToDeath(connection, 0);
    ...
    boolean removeSubscription(String id, ConnectionRecord connection, IBinder token) {
        try {
            if (token == null) {
                boolean z = connection.subscriptions.remove(id) != null;
                this.mCurConnection = connection;
                onUnsubscribe(id);
                this.mCurConnection = null;
                return z;
