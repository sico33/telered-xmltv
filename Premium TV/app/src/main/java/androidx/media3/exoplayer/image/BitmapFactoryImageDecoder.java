...
public final class BitmapFactoryImageDecoder extends SimpleDecoder<DecoderInputBuffer, ImageOutputBuffer, ImageDecoderException> implements ImageDecoder {
    ...
    public ImageDecoderException decode(DecoderInputBuffer inputBuffer, ImageOutputBuffer outputBuffer, boolean reset) {
        try {
            ...
            Assertions.checkState(inputData.hasArray());
            Assertions.checkArgument(inputData.arrayOffset() == 0);
    ...
    public static Bitmap decode(byte[] data, int length) throws ImageDecoderException {
        try {
            return BitmapUtil.decode(data, length, null);
        } catch (ParserException e) {
            throw new ImageDecoderException("Could not decode image data with BitmapFactory. (data.length = " + data.length + ", input length = " + length + ")", e);
