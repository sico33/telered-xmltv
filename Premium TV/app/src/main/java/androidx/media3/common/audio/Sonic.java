...
final class Sonic {
    ...
    public void queueEndOfStream() {
        ...
        this.inputFrameCount = xSample2 + (this.maxRequiredFrameCount * 2);
        processStreamInput();
    ...
    private int findPitchPeriod(short[] samples, int position) {
        ...
        if (this.channelCount == 1 && skip == 1) {
            ...
            int period2 = findPitchPeriodInRange(this.downSampleBuffer, 0, this.minPeriod / skip, this.maxPeriod / skip);
            if (skip == 1) {
                period = period2;
            } else {
        ...
        while (true) {
            if (position < this.pitchFrameCount - 1) {
                while ((this.oldRatePosition + 1) * newSampleRate > this.newRatePosition * oldSampleRate) {
                    ...
                    int i2 = this.newRatePosition;
                    this.newRatePosition = i2 + 1;
                ...
                int position2 = this.pitchFrameCount;
                removePitchFrames(position2 - 1);
                return;
            }
        }
    }
    ...
    private void changeSpeed(float speed) {
        ...
        do {
            if (this.remainingInputToCopyFrameCount > 0) {
                ...
                if (d > 1.0d) {
                    positionFrames += skipPitchPeriod(sArr, positionFrames, speed, period) + period;
                } else {
                    positionFrames += insertPitchPeriod(sArr, positionFrames, speed, period);
                }
            }
