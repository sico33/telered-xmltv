...
public final class ExifOrientationStream extends FilterInputStream {
    ...
    public int read() throws IOException {
        ...
        if (this.position < 2 || this.position > ORIENTATION_POSITION) {
            ...
            i = this.position == ORIENTATION_POSITION ? this.orientation : EXIF_SEGMENT[this.position - 2] & 255;
        }
        if (i != -1) {
            this.position++;
        }
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        if (this.position > ORIENTATION_POSITION) {
            ...
            if (i3 < 2) {
                iMin = super.read(bArr, i, 2 - i4);
            } else {
                iMin = Math.min(ORIENTATION_POSITION - i4, i2);
