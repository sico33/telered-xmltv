...
public final class Queues {
    ...
    public static <E> int drain(BlockingQueue<E> blockingQueue, Collection<? super E> collection, int i, long j, TimeUnit timeUnit) throws InterruptedException {
        ...
        while (iDrainTo < i) {
            ...
            if (iDrainTo < i) {
                E ePoll = blockingQueue.poll((jNanoTime + nanos) - System.nanoTime(), TimeUnit.NANOSECONDS);
                if (ePoll == null) {
                    break;
                }
        ...
        while (iDrainTo < i) {
            try {
                ...
                if (iDrainTo < i) {
                    while (true) {
                        try {
                            ePoll = blockingQueue.poll((jNanoTime + nanos) - System.nanoTime(), TimeUnit.NANOSECONDS);
                            break;
