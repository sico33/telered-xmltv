...
class ObjectCountHashMap<K> {
    ...
    class MapEntry extends Multisets.AbstractEntry<K> {
        ...
        final K key;
        int lastKnownIndex;
        final ObjectCountHashMap this$0;
        ...
        while (true) {
            if (getHash(this.entries[i2]) == i && Objects.equal(obj, this.keys[i2])) {
                ...
                if (i3 == -1) {
                    ...
                    this.entries[i3] = swapNext(this.entries[i3], getNext(this.entries[i2]));
                }
                moveLastEntry(i2);
                this.size--;
                this.modCount++;
    ...
    private void resizeTable(int i) {
        ...
        for (int i3 = 0; i3 < this.size; i3++) {
            ...
            iArrNewTable[i4] = i3;
            jArr[i3] = (((long) i5) & NEXT_MASK) | (((long) hash) << 32);
        }
        this.threshold = i2 + 1;
    ...
    private static long swapNext(long j, int i) {
        return (HASH_MASK & j) | (NEXT_MASK & ((long) i));
    }
    ...
    void moveLastEntry(int i) {
        ...
        objArr[i] = this.keys[size];
        this.values[i] = this.values[size];
        this.keys[size] = null;
        this.values[size] = 0;
        long j = this.entries[size];
        this.entries[i] = j;
        this.entries[size] = -1;
        int iHashTableMask = hashTableMask() & getHash(j);
        int i2 = this.table[iHashTableMask];
        if (i2 == size) {
            this.table[iHashTableMask] = i;
            return;
        }
        while (true) {
